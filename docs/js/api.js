...
;(() => {
    ...
    const { asyncForEach } = window.__LSCHAT__.UTILS
    const { name, random, lorem, date } = faker
    ...
    const MAIN_USER = new User('Leandro Simões', 'https://avatars3.githubusercontent.com/u/5066378?s=400&u=98d81da11220a6d0f7f51532e2c3e949b50a445b&v=4')
    
    const getRandomMessages = async (howMany = 1, users) => {
        const messages = []
    ...
    const getRandomUsers = async (howMany = 1) => {
        if (!howMany) return []
    ...
    const getPrevRandomMessages = async (users) => {
        END_DATE = START_DATE
